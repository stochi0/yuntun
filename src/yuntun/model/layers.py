...
class Qwen3Attention(nn.Module):
    ...
    def forward(
        ...
        k = k.view(bsz, q_len, self.num_key_value_heads_local, self.head_dim).transpose(1, 2)
        v = v.view(bsz, q_len, self.num_key_value_heads_local, self.head_dim).transpose(1, 2)
...
class Qwen3MLP(nn.Module):
    def __init__(self, config: Qwen3Config, tp_group=None):
        ...
        if self.tp_size > 1 and TP_AVAILABLE:
            self.gate_proj = ColumnParallelLinear(self.hidden_size, self.intermediate_size, bias=False, gather_output=False, tp_group=tp_group)
            self.up_proj = ColumnParallelLinear(self.hidden_size, self.intermediate_size, bias=False, gather_output=False, tp_group=tp_group)
            self.down_proj = RowParallelLinear(self.intermediate_size, self.hidden_size, bias=False, tp_group=tp_group)
        else:
